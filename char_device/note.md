...
你在 kernel module 裡會寫：

- **cdev_init()** → 初始化一個 `struct cdev`
- **cdev_add()** → 把這個 cdev 註冊到 Linux VFS 系統
- Linux 才知道這個 “設備節點背後是哪個 driver 的 read/write/open”

👉 **cdev 僅存在於 kernel 內，跟使用者空間無關！**

#### 📌 cdev 的功能

- 定義這個字元裝置的行為（read/write/open/release）
- 提供 `/dev/mydev` 背後真正的 driver 介面
- 在 VFS 架構中註冊一個可操作的 device

#### ✔ cdev = Kernel 用來管理字元裝置的資料結構與行為。跟 /dev/node、使用者空間無關。  
...
- 監控 kernel 發出的 uevent
- 自動建立 /dev/xxx 節點（不用你手動 mknod）
- 設定 device 權限（rw、group 等）
- 設定 symlink（例如 /dev/serial → /dev/ttyS0）

## cdev vs. udev
| 功能 | cdev | udev |
| --- | --- | --- |
| 層級 | Kernel Space | User Space |
| 作用 | 建立字元裝置的 driver 行為 (open/read/write) | 自動產生 /dev node |
| 誰建立？ | 驅動程式作者在 module 裡建立 | Linux 系統自動 |
| 負責的東西 | VFS 註冊、檔案操作回呼 | /dev node、權限、符號連結 |
| 是否一定要？ | ✔ 必要（字元裝置一定要 cdev） | ✖ 不一定（沒有 udev 要自己 mknod） |
| 跟 /sys 有關嗎 | 無關（但 device_create 會讓 /sys/class 產生） | 使用 uevent 從 /sys/class 裡讀取資訊 |  
...
##### - 方法二：自動建立（使用 udev + class_create/device_create）
...
在 Linux 中，cdev（character device object）是 **VFS 與你的 driver 之間的真正橋梁**。

要讓 `/dev/mydev` 能呼叫到你的 `open/read/write`，就需要建立 cdev。
...
```
register_chrdev()  
   ↓（kernel 內部 auto 建立 anonymous cdev）
建立 cdev（kernel 自動）
   ↓
你寫的 open/read/write 就被掛上
```
...
- 多個 minor（支援多裝置）
- 完全控制 lifecycle
- 所有正式 kernel driver 都使用這方式（BMC/嵌入式/平台 driver 一律如此）
